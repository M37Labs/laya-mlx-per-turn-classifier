...
CASES = [
    ...
    (
        "Can you send me a copy of last month's invoice when you get a chance?",
        "billing",
        False,
        "low",
    ),
    (
        "I cancelled my subscription but was still charged. I want my money back.",
        "billing",
        True,
        None,
    ),
    ...
    (
        "Production is down, none of our users can log in! Fix this now!",
        "technical",
        False,
        "high",
    ),
    (
        "The export button is slightly misaligned on mobile, no rush.",
        "technical",
        False,
        "low",
    ),
    (
        "I'd like to buy 50 more seats for my team. What's the pricing?",
        "sales",
        False,
        None,
    ),
    (
        "Do you offer an enterprise plan? Just exploring options for next year.",
        "sales",
        False,
        "low",
    ),
...
def test_department(results, msg, department, _refund, _urgency):
    assert results[msg]["department"]["choice"] == department, results[msg][
        "department"
    ]
...
@pytest.mark.parametrize(
    "msg,_department,_refund,urgency", [c for c in CASES if c[3] is not None]
)
